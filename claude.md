...
13. **Preferir `Bash` sobre `Write`/`Edit` para tocar archivos (7-ago-2026).** Al crear o modificar archivos de este proyecto, usar `Bash` (heredoc `cat > archivo << 'EOF' ... EOF`, `sed`, etc.) con un `description` claro, en vez de las herramientas `Write`/`Edit`. Razón: `Write`/`Edit` no tienen parámetro `description`, así que la ventana de permiso solo muestra un genérico "Allow write to X?" sin propósito visible; `Bash` sí lo muestra en el mismo lugar donde el usuario autoriza el cambio.
